...
public class HibernateHelper {
	...
	public List<List<Object>> executeQuery() throws InterruptedException {
		...
		if (!rowsList.isEmpty()){
			...
			// TODO
			String unixStr = "";
			if (rowsList.get(rowsList.size() - 1).get(0).toString().split("\\.").length > 1) {
				unixStr = rowsList.get(rowsList.size() - 1).get(0).toString().split("\\.")[0];
			} else {
				unixStr = rowsList.get(rowsList.size() - 1).get(0).toString();
			}
	...
	public List<List<Object>> executeQueryForDataUpdated(String SelectKeyColumn, String SelectKey) throws InterruptedException {
		...
		if (sqlSourceHelper.isCustomQuerySet()){
			...
			query = session.createSQLQuery(sqlSourceHelper.buildQuery(SelectKeyColumn, SelectKey));
